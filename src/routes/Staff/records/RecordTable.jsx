...
import { Search, Filter, XCircle, AlertCircle, CircleCheck, CheckCircle2, Printer, CalendarIcon } from "lucide-react";
...
const renderStatusBadge = (status) => {
    const iconMap = {
        Unpaid: <XCircle className="h-4 w-4 text-red-500 dark:text-red-400" />,
        Expired: <XCircle className="h-4 w-4 text-red-500 dark:text-red-400" />,
        Pending: <AlertCircle className="h-4 w-4 text-orange-500 dark:text-orange-400" />,
        Unclaimed: <AlertCircle className="h-4 w-4 text-orange-500 dark:text-orange-400" />,
        Washing: <CircleCheck className="h-4 w-4 text-blue-500 dark:text-blue-400" />,
...
const RecordTable = ({ items = [] }) => {
    ...
    const filtered = items.filter((r) => {
        ...
        const matchesPayment = selectedPayment.length === 0 || selectedPayment.includes(r.paymentStatus);
        const matchesPickup = selectedPickup.length === 0 || selectedPickup.includes(r.pickupStatus);
        return matchesSearch && matchesDate && matchesPayment && matchesPickup;
    });
                                    ...
                                    <div className="space-y-4">
                                            ...
                                            {statusFilters.payment.map((p) => (
                                                ...
                                                >
                                                    <Checkbox
                                                        ...
                                                        onCheckedChange={() => toggleStatus("payment", p)}
                                                    />
                                                    {p}
                                                </label>
                                            ))}
                                        ...
                                        <div>
                                            ...
                                            {statusFilters.pickup.map((p) => (
                                                ...
                                                >
                                                    <Checkbox
                                                        checked={selectedPickup.includes(p)}
                                                        onCheckedChange={() => toggleStatus("pickup", p)}
                                                    />
                                                    {p}
                                                </label>
                                            ))}
