...
const TrackingTable = ({
  ...
  console.log("📊 TrackingTable received jobs:", jobs);
  ...
  // Modified function to get visible loads - includes all non-completed loads
  const getVisibleLoads = (job, expanded) => {
    ...
    console.log(`👀 Job ${job.transactionId}: ${visibleLoads.length} visible loads (expanded: ${expanded})`);
          ...
          <tbody>
            <AnimatePresence>
              {jobs.map((job) => {
                ...
                console.log(`🎯 Rendering job ${job.transactionId} with ${visibleLoads.length} visible loads`);

                if (visibleLoads.length === 0) {
                  console.log(`⏭️ Skipping job ${job.transactionId} - no visible loads`);
                  return null;
                }
                  ...
                  <React.Fragment key={jobKey}>
                    {visibleLoads.map((load, i) => {
                      const originalIndex = job.loads.findIndex(
                        (l) => l.loadNumber === load.loadNumber
                      );
                      ...
                      console.log(`  📦 Load ${load.loadNumber}: ${load.status}, machineType: ${machineType}`);
