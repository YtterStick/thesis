...
import { MoreVertical, Plus, Pencil, Trash2, XCircle, AlertCircle, CircleCheck, CheckCircle2 } from "lucide-react";
                            ...
                            {items.length > 0 ? (
                                items.map((item) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            <td className={cn("p-2 font-medium", getRoleTextColor(item.role))}>{item.name}</td>
                                            <td className="flex items-center p-2 text-slate-600 dark:text-slate-300">
                                                {quantity} {unit}
                                                {statusIcon}
                                            </td>
                                            <td className="p-2 text-slate-600 dark:text-slate-300">₱{item.price?.toFixed(2) ?? "—"}</td>
                                            <td className="p-2 text-xs text-slate-500 dark:text-slate-400">
                                                {item.lastRestock ? new Date(item.lastRestock).toLocaleString() : "—"}
                                            </td>
