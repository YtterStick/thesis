    ...
    return (
        <Tooltip>
                ...
                >
                    ...
                    {machine.needsMaintenance && (
                        <button
                            ...
                            className="absolute left-2 top-2 rounded-full bg-yellow-100 p-1 text-yellow-600 transition-colors hover:bg-yellow-200"
                    ...
                    <div className="space-y-2 text-center text-sm text-slate-700 dark:text-slate-300">
                        ...
                        {machine.needsMaintenance && <div className="text-xs text-yellow-600 dark:text-yellow-400">⚠️ Needs Maintenance</div>}
                    </div>
                </div>
            </TooltipTrigger>
            <TooltipContent className="text-xs">
                ...
                {machine.lastMaintenance && <div>🔧 Last Maintenance: {new Date(machine.lastMaintenance).toLocaleDateString()}</div>}
                {machine.nextMaintenance && <div>📅 Next Maintenance: {new Date(machine.nextMaintenance).toLocaleDateString()}</div>}
            </TooltipContent>
        </Tooltip>
    );
}
...
export default function MachineMainPage() {
    ...
    const postponeMaintenance = () => {
        ...
        setMachines((prev) =>
            prev.map((m) =>
                m.id === maintenanceReminder.id ? { ...m, nextMaintenance: nextReminderDate.toISOString(), needsMaintenance: false } : m,
            ),
        );
            ...
            >
                <DialogContent>
                    ...
                    <DialogFooter className="gap-2 sm:gap-0">
                        <Button
                            variant="outline"
                            onClick={postponeMaintenance}
                        >
                            Remind me in 3 days
                        </Button>
                        <Button onClick={() => maintenanceReminder && handleMaintenance(maintenanceReminder)}>Start Maintenance</Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
